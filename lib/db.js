...
export async function initDb() {
  ...
  await db`ALTER TABLE trips ADD COLUMN IF NOT EXISTS encoded_polyline TEXT`;
  await db`ALTER TABLE trips ADD COLUMN IF NOT EXISTS loc_times JSONB`;
  await db`ALTER TABLE trips ADD COLUMN IF NOT EXISTS loc_times_computed_at TIMESTAMPTZ`;
  // Vehicle location pings from Zubie webhooks — used for crossing time interpolation
  await db`
  ...
  `;
  await db`
    CREATE INDEX IF NOT EXISTS idx_vehicle_pings_vehicle_time
    ON vehicle_pings (vehicle_key, timestamp)
  `;
  return { ok: true };
}

// Save interpolated loc_times for a trip (timestamped polyline points)
export async function saveTripLocTimes(trip_key, locTimes) {
  const db = sql();
  await db`
    UPDATE trips
    SET loc_times = ${JSON.stringify(locTimes)},
        loc_times_computed_at = NOW()
    WHERE trip_key = ${trip_key}
  `;
}

// Store a single location ping from a vehicle_location_update webhook
export async function saveVehiclePing(ping) {
  const db = sql();
  await db`
    INSERT INTO vehicle_pings (vehicle_key, timestamp, lat, lng, speed_mph, heading, raw)
    VALUES (
      ${ping.vehicle_key},
      ${ping.timestamp},
      ${ping.lat},
      ...
      ${ping.speed_mph ?? null},
      ${ping.heading ?? null},
      ${JSON.stringify(ping.raw ?? {})}
    )
  `;
}

// Fetch pings for a vehicle within a time window (for trip crossing interpolation)
export async function getPingsForTrip(vehicle_key, started_at, ended_at) {
  const db = sql();
  return db`
